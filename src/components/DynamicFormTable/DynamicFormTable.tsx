...
const DynamicFormTable: React.FC<DynamicFormTableProps> = ({
  ...
  const { sortField, sortDirection, handleSort, getSortedData } =
    useTableSort();
        ...
        >
              ...
              {columns.map((col) => (
                <TableHeaderCell
                  ...
                  onSort={(columnId) =>
                    handleSort(
                      columnId,
                      !col.isActionColumn && col.sortable !== false
                    )
                  }
                />
              ))}
          ...
          <TableBody>
            {sortedData.map((row, rowIndex) => (
              <TableRow
                key={rowIndex}
                ref={rowIndex === sortedData.length - 1 ? lastElementRef : null}
                sx={{
